...
public class SolutionsV2 {
    ...
    // This the solution from leetcode, same approach as v2 where we are using the submatrix to find the kth smallest item.
    // The catch here is we update the start/end with values found during the search for sub matrix. Not this reduces
    // the time complexity significantly for large extreme value for example
    // if the mat[0][0]= 0 & mat[n-1][n-1] = 2147483647 think how long it will take converge to the solution of
    // if we always calc mid using extreme values. Better option is to use it only once and then from there while
    // looking for the submatrix calc the value low or high to be used
    ...
    public int kthSmallestV3(int[][] matrix, int k) {
        ...
        while (start < end) {
            ...
            if (count < k) start = smallLargePair[1]; // search higher which is same as start = mid +1 in binary search
            else end = smallLargePair[0]; // search lower which is same as high = mid -1 in binary search
        }
        return start;
    }
